import { Radio, CalendarCheck, TagIcon, ListIcon, Calendar, RefreshCcw } from "lucide-react";
...
interface UserPodcastCardProps {
  podcast: PodcastDetailResponse;
}

export default function UserPodcastCard({ podcast }: UserPodcastCardProps) {
      ...
      <div className="space-y-3 p-4 pt-3 md:p-6 md:pt-4" id={`podcast-detail-${podcast.podcastId}`}>
        ...
        <div className="space-y-2 text-xs md:text-sm">
          ...
          <div className="flex flex-col gap-x-4 gap-y-2 md:flex-row md:flex-wrap">
            ...
            <div className="text-muted-foreground flex items-center gap-1.5">
              <RefreshCcw className="h-3.5 w-3.5 shrink-0 md:h-4 md:w-4" aria-hidden="true" />
              <CardDescription className="text-xs md:text-sm">
