...
export default function AdminPodcastGridCard({ podcast }: AdminPodcastGridCardProps) {
  const router = useRouter();

  const handlePodcastClick = () => {
    router.push(`/admin/podcasts/${podcast.podcastId}`);
  };

  const handleCreatorClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    router.push(`/admin/users/${podcast.creatorId}`);
  };
