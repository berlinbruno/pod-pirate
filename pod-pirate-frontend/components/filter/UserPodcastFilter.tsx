...
interface UserPodcastFilterProps {
  initialSearch: string;
  initialStatus: string;
}

export default function UserPodcastFilter({
  initialSearch,
  initialStatus,
}: UserPodcastFilterProps) {
  ...
  const updateURL = (updates: { search?: string; status?: string; page?: number }) => {
    ...
    router.push(queryString ? `/dashboard/podcasts?${queryString}` : "/dashboard/podcasts", {
      scroll: false,
    });
  };
  ...
  const handleSearch = (query: string) => {
    ...
    if (!trimmed) {
      router.push("/dashboard/podcasts", { scroll: false });
      return;
    }
